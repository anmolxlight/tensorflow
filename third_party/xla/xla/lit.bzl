...
def lit_test_gpu_runtime(
        ...
        backend_args = {},
        **kwargs):
    """Creates lit test suites per GPU backend.
    ...
    # NOTE: This processing logic is **heavily** based on xla_test processing
    # with simplifications: only GPU, no multidevice and dropping some options
    # that seem unrelated to lit tests.

    for backend in backends:
        if backend not in GPU_BACKENDS and backend != "gpu":
            fail("%s is not a GPU backend" % backend)
    backends, disabled_backends, backend_tags, backend_args = prepare_gpu_backend_data(backends, [], backend_tags, backend_args, tags)
    backends = [
        backend
        for backend in backends
